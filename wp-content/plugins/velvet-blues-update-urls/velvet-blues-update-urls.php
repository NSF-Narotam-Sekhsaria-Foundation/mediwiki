...
function VelvetBluesUU_management_page(){
	if ( !function_exists( 'VB_update_urls' ) ) {
		function VB_update_urls($options,$oldurl,$newurl){	
			...
			'custom_options' =>			array("UPDATE $wpdb->options SET option_value = replace(option_value, %s, %s)",  __('Option Fields','velvet-blues-update-urls') ),
				...
				elseif ( $option == 'custom_options' ) {
					...
					for( $page = 0; $page < $pages; $page++ ) {
						...
						if( trim($value) == '' )
							...
							if( $edited != $value ){
								$fix = $wpdb->query("UPDATE $wpdb->options SET option_value = '".$edited."' WHERE option_id = ".$item->option_id );
								if( $fix )
									$n++;
							}
						}
			...
			<table class="form-table">
				<tr>
					<td><p style="line-height:20px;">
							...
							<br/>
							<input name="VBUU_update_links[]" type="checkbox" id="VBUU_update_true44" value="custom_options" />
							<label for="VBUU_update_true44"><strong>
								<?php _e('URLs in option fields','velvet-blues-update-urls'); ?>
								</strong></label>
							<br/>
